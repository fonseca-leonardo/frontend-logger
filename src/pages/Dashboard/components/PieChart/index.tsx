...
function CustomCursor({ payload, label, active, ...rest}: any) {
  if (active) {
    console.log({...rest})
    return (
...
const PieChart: React.FC<Props> = ({statusCodeReport}) => {
    ...
    const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index }: any) => {
        ...
        return (
          <text x={x} y={y} fill={theme.fontColor} style={{ fontWeight: 'bold' }} textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central">
    ...
    return (
      <Card style={{ marginLeft: 32, width: "33.333%", minWidth: 250 }}>
        <span style={{ fontSize: 16, margin: '8px 0px'}}>Contador de status code</span>
              ...
              >
                  {
                      statusCodeReport.data.map(el => (
                          <Cell key={el.statusCode} fill={PieColors[el.statusCode]} />
                      ))
                  }
              </Pie>
              <Tooltip
                  contentStyle={{ background: theme.background, width: 200 }}
